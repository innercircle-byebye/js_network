	...
	for (int i = 0; i < events; ++i) {
		...
		if (event_list_[i].flags & EV_ERROR) {
			...
			if (c->getListen()) {
				...
				// event_handler.set_request_message(c->buffer_);
				// event_handler.test_print_origin_message();
				// kqueue_set_event(c, EVFILT_WRITE, EV_ADD | EV_ONESHOT);

				// buffer_ 이어붙이는 기능
				// if (event_handler.set_request_message(c->buffer_))
				if (event_handler.appendBufferToRequestMessage(c->buffer_))
				{
					event_handler.parseReqMsg();
					// c->set_data(event_handler.get_req_msg());
					c->setRequestMessage(event_handler.getReqMsg());
					event_handler.testPrintOriginMessage();
        ...
        kqueueSetEvent(c, EVFILT_WRITE, EV_ADD | EV_ONESHOT);
			...
			if (event_list_[i].flags & EV_EOF) {
				...
				event_handler.processEvent(res_msg, c->getRequestMessage(), c->getSockaddrToConnect());
		std::cout << "===test print res_mes===" << std::endl;
		std::cout << res_msg << std::endl;
				// std::string res_msg = "HTTP/1.1 200 OK\r\nServer: jsnetwork\r\nContent-Length: 31\r\nContent-Type: text/html\r\n\r\n<!DOCTYPE html>\n<html>\n</html>\n";
				send(event_list_[i].ident, res_msg.c_str(), res_msg.size(), 0);
			}
		}
	}
}
